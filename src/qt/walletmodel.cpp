...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedAnonBal(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0),
...
qint64 WalletModel::getAnonBalance() const
{
    return wallet->GetAnonBalance();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    qint64 newAnonBal = getAnonBalance();
    qint64 newStake = getStake();
    qint64 newUnconfirmedBalance = getUnconfirmedBalance();
    qint64 newImmatureBalance = getImmatureBalance();

    if (cachedBalance != newBalance
        || cachedAnonBal != newAnonBal
        ...
        || cachedImmatureBalance != newImmatureBalance)
    {
        ...
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedImmatureBalance = newImmatureBalance;
        emit balanceChanged(newBalance, newAnonBal, newStake, newUnconfirmedBalance, newImmatureBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("TogetherChain is not connected!"));
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {
            case TXT_BTC_TO_BTC:
            case TXT_BTC_TO_ANON:
                inputType = 0;
                break;
            case TXT_ANON_TO_ANON:
            case TXT_ANON_TO_BTC:
                inputType = 1;
                break;
            default:
    ...
    {
        nBalance = wallet->GetAnonBalance();
        if ((nTotalOut + MIN_TX_FEE_ANON) > nBalance)
            return SendCoinsReturn(SCR_AmountWithFeeExceedsAnonBalance, MIN_TX_FEE_ANON);
    };

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.txnTypeInd == TXT_BTC_TO_BTC
                || rcp.txnTypeInd == TXT_ANON_TO_BTC)
            {
                // -- out unc
                std::string sError;
