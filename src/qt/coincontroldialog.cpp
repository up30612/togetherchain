...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog, BitcoinBridge *bridge)
{
    if (!model) return;
    ...
    } else
    {
        ...
        l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : ""); // Change < 0.01TGC
